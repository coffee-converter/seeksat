...
export async function ogResponse(req) {
  ...
  try {
    ...
    // 1h CDN cache. Kept short on purpose: an explicit-pass image is
    // recomputed against the live TLE and we want chart corrections (and
    // re-shares of the same link) to propagate within the hour rather than
    // pinning a day-old render. stale-while-revalidate serves the cached
    // copy instantly while the next request refreshes it in the background.
