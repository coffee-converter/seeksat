...
const _onPanelToggle = () => document.body.classList.toggle("panel-collapsed");
_panelToggle?.addEventListener("click", _onPanelToggle);
...
function renderObsList() {
  // No-op: ObservationsPanel renders the obs-list. Kept as a stub
  // because renderFromObserverButtons is called by the renderObsList
  // signature elsewhere; calling it here keeps the "From <obs>"
  // camera buttons in sync when observations change.
...
cameraCtrl.frameAll(); // initial framing only; edits afterward leave the view alone
  ...
  return () => {
    _unsubTle();
    _unsubBridge();
    _panelToggle?.removeEventListener("click", _onPanelToggle);
