// lib/mcp/tle-fetch.mjs - fetch a current TLE for ANY NORAD id (server
// side). Mirrors the client source ladder in lib/pass-finder/tle.js
// (now also id-parameterized) but with an injectable fetch so the
// source-ladder logic is unit-testable without network. Source order
// and parse shapes match the client module.
