...
export async function getPassChartTool(input, deps, tier = 'free') {
  const { entry, record } = await requireRecord(input.satellite, deps, tier);
  const loc = await resolveLocation(input, deps);
  // Label the observer by its geocoded place name, or the coordinates when
  // the agent passed raw lat/lon — used in both the chart title and summary.
  const name = loc.displayName ?? `${loc.latDeg.toFixed(2)}, ${loc.lonDeg.toFixed(2)}`;
  const observer = { name, latDeg: loc.latDeg, lonDeg: loc.lonDeg };
