...
export async function renderPassChartPng({ entry, record, observer, mode = 'visual', nowMs, scanHours = 72 }) {
  ...
  // Short, title-friendly satellite name ("ISS", "Tiangong", "Hubble").
  const satLabel = entry.shortName ?? entry.name;
  const pass = nextPassWindow({
    line1: record.line1, line2: record.line2, observer,
    startMs: nowMs, windowHours: scanHours, mode,
  });
  if (!pass) {
    return { summary: `No upcoming ${mode} pass for ${satLabel} from ${loc} in the next ${scanHours}h.` };
  }
  ...
  const svg = renderPassChartSVG({
    observer, win: pass.win, peakMs: pass.peakMs, issEcefAt: sampler, satName: satLabel, full: true,
  });
  ...
  const summary =
    `${satLabel} over ${loc}: rises ${new Date(pass.win.startMs).toISOString()}, ` +
