...
import {
  regionForObservers, buildGrid, computeCellMetrics, buildCloudInterpolator,
...
// React's WindowsList clicks (and the Now button) call store.selectWindow,
// which bumps windowSelectNonce. We react ONLY to that nonce so an explicit
// user pick always runs the jump (camera, clock, entity reframe) — even when
// the clicked row is already the soft-tracked active window (the clock parks
// inside the upcoming pass, so its index is already in the store). The
// per-frame auto-tracker writes activeWindowIdx via setActiveWindowIdx with
// NO nonce bump, so it updates the highlight without triggering a jump, and
// jumpToWindow's own setActiveWindowIdx mirror can't re-enter this handler.
const _unsubActiveWindow = usePassFinderStore.subscribe((s, prev) => {
  if (s.windowSelectNonce === prev.windowSelectNonce) return;
  const idx = s.activeWindowIdx;
  if (idx < 0) {
    state.activeWindowIdx = -1;
  ...
  if (state.windows[idx]) jumpToWindow(idx);
});
...
// all cells query the SAME predicate timestamps, so this collapses a
// huge amount of repeated SGP4 work. Cleared each run.
...
async function runHeatmap() {
  ...
  try {
    ...
    const cellDeps = {
      nowMs: Date.now(),
      windowMs: store.heatmapWindowDays * 86_400_000,
    ...
    // Fan the per-cell scoring out one grid ROW at a time, yielding to
    // the event loop between rows. The geometry sweep is heavy (576
    // cells × thousands of timesteps), so running it all synchronously
    // would freeze the page; chunking keeps the UI live and — because we
    // re-check runId after each yield — makes an in-flight compute
    // actually interruptible when the user toggles off or changes the
    // window/metric.
    const metrics = [];
    let maxCount = 0;
    for (let row = 0; row < grid.n; row++) {
      for (let col = 0; col < grid.n; col++) {
        const cell = grid.cells[row * grid.n + col];
        const m = computeCellMetrics(cell, cellDeps);
        if (m.count > maxCount) maxCount = m.count;
        metrics.push({ row: cell.row, col: cell.col, ...m });
      }
      await new Promise((resolve) => setTimeout(resolve));
      if (runId !== heatmapRunId) return; // superseded mid-sweep
    }
    ...
    // Only the run that still owns the slot clears the spinner — a
    // superseded run must not stomp the newer run's "computing" state.
    if (runId === heatmapRunId) store.setHeatmapComputing(false);
  }
}
