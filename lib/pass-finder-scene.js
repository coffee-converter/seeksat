...
function ensureGpLineEntity() {
  ...
  gpLineEntity = viewer.entities.add({
    polyline: {
      ...
      // that fail the depth test (i.e., are behind Earth). One third
      // of the normal alpha gives the X-ray through-Earth look the
      // user asked for without obscuring the actual visible portion.
      depthFailMaterial: new Cesium.PolylineDashMaterialProperty({
        color: Cesium.Color.fromCssColorString("#cfe0ff").withAlpha(0.15),
        dashLength: 10,
      }),
      arcType: Cesium.ArcType.NONE,
    },
...
function ensureOrbitEntity() {
  ...
  orbitEntity = viewer.entities.add({
    polyline: {
      ...
      // still render at ~33% of the in-front alpha so the full ring
      // is visible at all times.
      depthFailMaterial: new Cesium.PolylineDashMaterialProperty({
        color: Cesium.Color.fromCssColorString("#7eb8ff").withAlpha(0.15),
        dashLength: 12,
      }),
      arcType: Cesium.ArcType.NONE,
    },
