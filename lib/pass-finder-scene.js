...
// Observer cards (header + buttons) are React-rendered by
// components/passes/ObserverCard.tsx. Each card exposes a polar slot
// div and calls window.__passesMountPolar to ask us to mount the
// SVG. We also handle card-level CustomEvents (remove, fps-toggle,
// open-polar-modal) here.
//
...
window.__passesMountPolar = function (el, obsId) {
  if (!el) return;
  const obs = state.observers.find(o => o.id === obsId);
  if (!obs) return;
  const svg = buildPolarPlot(obs);
  updatePolarPlotArc(svg, obs);
...
const _onPassesOpenPolarModal = (ev) => {
  const id = ev.detail?.obsId;
  if (id) openPolarModal(id);
};
...
window.addEventListener("passes-open-polar-modal", _onPassesOpenPolarModal);
...
// Play / Pause / Reset migrated to components/passes/PlaybackControls
// .tsx — clock manipulation happens there directly. The Reset button
// also clears the active window selection, which still lives in
// state.activeWindowIdx; PlaybackControls dispatches a `passes-reset`
// CustomEvent on the window so we can do that side-effect here.
const _onPassesReset = () => {
  state.activeWindowIdx = -1;
  renderWindowsList();
};
window.addEventListener("passes-reset", _onPassesReset);
...
const cameraCtrl = wireCameraControls(viewer, {
  ...
  return () => {
    ...
    _unsubTle();
    _unsubClickToPlace();
    window.removeEventListener("passes-reset", _onPassesReset);
    ...
    window.removeEventListener("passes-open-polar-modal", _onPassesOpenPolarModal);
    delete window.__passesMountPolar;
    if (_minElevDebounce) clearTimeout(_minElevDebounce);
