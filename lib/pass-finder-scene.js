...
// Display name of the currently-selected satellite. Prefers the curated
// catalog name (keyed by the store's selectedNoradId) so it matches the
// selector; falls back to the raw TLE name, then a generic label. Used by
// the globe dot label and the polar-chart title so they track the
// selection instead of being hardwired to "ISS".
function selectedSatName() {
...
// Compact name for the globe dot label — the curated catalog shortName
// ("ISS", "Tiangong", "Hubble"), falling back to a parenthesis-stripped
// TLE name when there's no catalog entry.
function selectedSatShortName() {
  const id = usePassFinderStore.getState().selectedNoradId;
  const entry = CATALOG.find((s) => s.noradId === id);
  if (entry?.shortName) return entry.shortName;
  return selectedSatName().replace(/\s*\([^)]*\)\s*$/, "").trim() || "Satellite";
}
  ...
  issEntity = viewer.entities.add({
    ...
    label: {
      ...
      text: new Cesium.CallbackProperty((time) => {
        const d = Cesium.JulianDate.toDate(time);
        const p = issEcefAt(d);
        if (!p) return selectedSatShortName();
        const cart = Cesium.Cartographic.fromCartesian(
          Cesium.Cartesian3.fromElements(p[0], p[1], p[2])
        );
        return `${selectedSatShortName()} · ${(cart.height / 1000).toFixed(0)} km`;
      }, false),
