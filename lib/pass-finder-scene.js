...
const cameraCtrl = wireCameraControls(viewer, {
  ...
  return () => {
    // Viewer disposal is owned by useCesiumViewer's cleanup. Any
    // module-local subscriptions the pass-finder scene needs to
    // detach get unhooked here (none yet — the imperative island
    // owns its own DOM listeners and they live on JSX-rendered
    // elements that React tears down on unmount).
  };
}
