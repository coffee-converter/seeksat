...
import {
  EVENT_STYLE, EVENT_COLS,
  EVENT_ROW_TITLE_Y, EVENT_ROW_TIME_Y, EVENT_ROW_POS_Y,
  diamondVerts, diamondPath, splitDiamondPaths,
...
import {
  localTimeScore,
  worstLocalTimeScore as _worstLocalTimeScorePure,
...
// now live in pass-finder/window-scoring.js. localTimeScore is
// re-exported as-is (pure); the others get thin wrappers binding
// state.observers + state.cloudForecasts + issEcefAt.
function worstLocalTimeScore(peakMs) {
  return _worstLocalTimeScorePure(peakMs, state.observers);
}
...
const cameraCtrl = wireCameraControls(viewer, {
  ...
  // assignment swaps in all four functions atomically; teardown
  // clears the slot.
  setSceneBridge({
