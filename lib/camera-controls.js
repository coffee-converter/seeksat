...
export function wireCameraControls(viewer, {
  ...
  // Point the custom wheel-zoom slides toward/away from. Default is
  // Earth's center; updated to the frame-bounding-sphere center on
  // Frame/Top, and to the anchor position on lockAnchor. With this,
  // zooming after clicking Frame keeps the framed scene centered on
  // screen instead of pulling away to a globe-centered radial view.
  let zoomAnchor = Cesium.Cartesian3.clone(Cesium.Cartesian3.ZERO);
  ...
  function lockAnchor() {
    ...
    anchorLocked = true;
    zoomAnchor = Cesium.Cartesian3.clone(anchor);
  ...
  function frameAll() {
    ...
    if (positions.length === 0) {
      zoomAnchor = Cesium.Cartesian3.clone(Cesium.Cartesian3.ZERO);
      viewer.camera.flyHome(1.2);
      return;
    }
    if (positions.length === 1) {
      zoomAnchor = Cesium.Cartesian3.clone(positions[0]);
    ...
    zoomAnchor = Cesium.Cartesian3.clone(bs.center);
  ...
  function topDown() {
    ...
    // Set the zoom anchor before computing camera position; whichever
    // branch below executes, `center` resolves to the scene-relevant
    // focus point we want subsequent wheel zooms to track.
    let altitude;
    ...
    zoomAnchor = Cesium.Cartesian3.clone(center);
  ...
  // Replace Cesium's default cursor-anchored wheel zoom with one
  // anchored on Earth's center. Cesium's default moves the camera
  // toward/away from whatever the cursor is hovering over — fine for
  // a CAD-style scene but disorienting on a globe (cursor at bottom
  // of screen + zoom out → Earth drops below the viewport because
  // the camera tilts to keep "under cursor" stationary). Our custom
  // handler slides the camera radially in/out from the globe center,
  // preserving the current view orientation so the globe never
  // drifts off screen as a side effect of zooming.
  viewer.scene.screenSpaceCameraController.enableZoom = false;
  const EARTH_R = 6378137;
  const ZOOM_MIN_DIST = EARTH_R + 1_000;      // 1 km above surface
  const ZOOM_MAX_DIST = EARTH_R * 8;          // ~50,000 km from center
  // Per-pixel-of-deltaY zoom factor. Trackpads emit deltaY in the
  // 1–10 range per micro-event (often dozens per second during a
  // gesture); a fixed-factor-per-event approach compounded fast and
  // felt hyper-sensitive. Exponential scaling keeps the response
  // proportional to the actual gesture size:
  //   single notch deltaY ≈ 100  → factor ≈ exp(100 × 0.0005) = 1.05  (5%)
  //   trackpad pixel deltaY ≈ 10 → factor ≈ exp(10  × 0.0005) = 1.005 (~0.5%)
  // Clamp the per-event delta so a single huge wheel event can't
  // teleport the camera.
  const ZOOM_EXP_PER_DELTA = 0.0005;
  const ZOOM_DELTA_CLAMP = 300; // ~3 wheel notches max per event
  viewer.canvas.addEventListener("wheel", (ev) => {
    ev.preventDefault();
    const cam = viewer.camera;
    // Vector from the current zoom anchor to the camera. Anchor is
    // Earth's center by default; updated by Frame / Top / lockAnchor
    // so post-Frame zooming tracks the framed scene, not the planet
    // as a whole.
    const fromAnchor = Cesium.Cartesian3.subtract(
      cam.positionWC, zoomAnchor, new Cesium.Cartesian3(),
    );
    const currentDist = Cesium.Cartesian3.magnitude(fromAnchor);
    if (currentDist < 1) return; // degenerate; nothing to zoom along
    const d = Math.max(-ZOOM_DELTA_CLAMP, Math.min(ZOOM_DELTA_CLAMP, ev.deltaY));
    const factor = Math.exp(d * ZOOM_EXP_PER_DELTA);
    // Min distance is still defined in earth-surface terms so we
    // never punch the camera through the globe — clamp the distance
    // FROM EARTH CENTER, not the distance from the anchor (which
    // could be a low-altitude point and let the camera scrape the
    // surface).
    const targetCenterDist = Math.max(
      ZOOM_MIN_DIST,
      Math.min(ZOOM_MAX_DIST, Cesium.Cartesian3.magnitude(cam.positionWC) * factor),
    );
    const targetCenterFactor = targetCenterDist / Cesium.Cartesian3.magnitude(cam.positionWC);
    const targetDist = currentDist * targetCenterFactor;
    const delta = targetDist - currentDist; // positive = away from anchor
    if (delta === 0) return;
    // Move the camera along its (camera − anchor) direction by delta.
    // View direction is untouched.
    const moveDir = Cesium.Cartesian3.normalize(fromAnchor, new Cesium.Cartesian3());
    cam.move(moveDir, delta);
  }, { passive: false });
