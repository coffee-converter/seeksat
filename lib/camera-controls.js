...
export function wireCameraControls(viewer, {
  ...
  const ZOOM_FACTOR   = 1.18;                 // ~18% per wheel notch
  viewer.canvas.addEventListener("wheel", (ev) => {
    ev.preventDefault();
    const cam = viewer.camera;
    // Vector from Earth's center to the camera (ECEF origin is the
    // globe center, so camera.positionWC = camera-from-center).
    const toCam = cam.positionWC;
    const currentDist = Cesium.Cartesian3.magnitude(toCam);
    const factor = ev.deltaY > 0 ? ZOOM_FACTOR : 1 / ZOOM_FACTOR;
    const targetDist = Math.max(
      ZOOM_MIN_DIST,
      Math.min(ZOOM_MAX_DIST, currentDist * factor),
    );
    const delta = targetDist - currentDist; // positive = away from earth
    if (delta === 0) return;
    // Move the camera along its own outward radial direction by delta
    // meters. View direction is untouched.
    const moveDir = Cesium.Cartesian3.normalize(toCam, new Cesium.Cartesian3());
